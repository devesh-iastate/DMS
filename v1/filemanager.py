...
def list_categories():
    if not os.path.exists(data_directory):
        return 503, "Data directory not reachable. Please contact admin."
    datasets = []
...
def filter_creator(category, train_size, test_size, validation_size, sample_size, user):
    ...
    if not category_exists:
        return 404, "Category not found."
    if sample_size == 0:
        return 412, "Sample size cannot be 0."
    if test_size+train_size+validation_size > 1:
        return 412, "Sum of proportions cannot be greater than 1."
    if test_size+train_size+validation_size <= 0:
        return 412, "Sum of proportions cannot be less than or equal to 0."
    ...
    filterColl.insert_one({"_id": new_id, "dataSize": count, "testList": test_array, "trainList": train_array,
                           ...
                           "validationPointer": 0, "validationList": validation_array, "timestamp": datetime.utcnow()})
...
def files_aggregator(filter_id, data_type, fetch_meta_data):
    ...
    if data_type == 'test':
        ...
        return 412, "invalid data type. Please enter test/train/validation"
    ...
    if len(filter_data) == 0:
        return 404, "Filter id " + str(filter_id) + " not found"
    ...
    if len(filter_list) < sample_size:
        return 412, "Sample size cannot be greater than the number of files in the dataset"
    if not os.path.exists(data_directory):
        return 404, "Data directory not found."
